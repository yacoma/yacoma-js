...
function addDepth(a: StringElementType): GroupElementType {
  return Object.assign({} as GroupElementType, a, {
    i: {
      [makeRef()]: {
        t: 'T',
        p: 0,
...
function moveInPara(
  ...
  const target = composition.g[targetName]

  if (typeof target.i === 'string') {
    // FIXME: Typescript should know better...
    const starget = target as StringElementType
...
function mergeFlatEnd(
  ...
  if (
    canFuse(start) &&
    canFuse(end) &&
    (start.elem.t === 'T' || start.path.length === 1)
  ) {
    ...
    if (hasDelete) {
      ...
      ops.push({
        op: 'select',
        value: caretSelection(start.path, startElem.i.length, position),
      })
    }
    ...
    if (!hasDelete) {
      ops.push({
        op: 'select',
        value: caretSelection([parentRef, ref], 0, position),
      })
    }
  } else {
...
function mergeSamePara(
  ...
  if (canFuse(start) && canFuse(end)) {
    ...
    const parent = composition.g[start.path[0]]
    if (isGroupElement(parent)) {
      let minPos = Infinity
      let maxPos = -1
      const parentKeys = Object.keys(parent.g)
      parentKeys.forEach(ref => {
        const p = parent.g[ref].p
        if (p < minPos) {
          minPos = p
        }
        if (p > maxPos) {
      ...
      if (start.elem.p === minPos && end.elem.p === maxPos) {
        ...
        ops.push({
          op: 'select',
          value: caretSelection(path, start.elem.i.length, position),
        })
      } else {
        // fuse in start
        ops.push({
...
function mergeTwoPara(
  ...
  const children = composition.g[endParaPath[0]].g as ElementsType
  let skip = true
      ...
      if (ref === endRef) {
        if (canFuse(start) && canFuse(end)) {
          ...
          if (hasDelete) {
            ...
            ops.push({
              op: 'select',
              value: caretSelection(start.path, start.elem.i.length, position),
            })
          }
          skip = false
          return
